...
# # Random Algorithm (Pooled Pauli, 2 Qubits)
...
def random_algorithm(initial_state, problem_hamiltonian, num_iterations, step_size, options, num_qubits):
    ...
    for iteration in range(num_iterations):
        ...
        theta = Parameter("theta") #Parameter for adapative unitary
        
        random_pauli_string = create_random_pauli_strings(num_qubits, num_strings=1, seed=iteration)[0]
        adaptive_unitary.append(PauliEvolutionGate(Pauli(random_pauli_string),
                                                   time=theta/2,
