...
def random_algorithm(initial_state, problem_hamiltonian, num_iterations, step_size, estimator, num_qubits):
    ...
    for iteration in range(num_iterations):
        ...
        random_unitary_matrix = random_clifford(num_qubits, seed=iteration) #Vk
        adaptive_unitary.append(random_unitary_matrix.adjoint().to_instruction(), range(num_qubits))
        adaptive_unitary.rx(theta, 0)
        adaptive_unitary.append(random_unitary_matrix.to_instruction(), range(num_qubits))
